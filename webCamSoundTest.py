...
# ---- Overlay Video Thread Variables ----
overlay_frame = None
overlay_lock = threading.Lock()
current_video_time = 0.0  # Global video time
...
def is_pressed(landmarks, button):
    ...
    return x < fingertip[0] < x + w and y < fingertip[1] < y + h
...
# ---- Thread: Overlay Video Player ----
def overlay_video_thread(video_path):
    global overlay_frame, current_video_time
    ...
    while True:
        ...
        with overlay_lock:
            overlay_frame = frame
            current_video_time = video.get(cv2.CAP_PROP_POS_MSEC) / 1000.0
        time.sleep(1 / fps)

# ---- Thread: Note Player from Queue ----
def note_player_thread():
    while True:
        key = note_queue.get()
        play_note(key)
...
threading.Thread(target=note_player_thread, daemon=True).start()
...
while True:
    ...
    # Detect hand touches (optional: you can disable if you want only scheduled notes)
    ...
    if current_overlay is not None:
        img = cv2.addWeighted(img, 1.0, current_overlay, 0.6, 0)

    # --- 🎶 Scheduled Note Playback ---
    for note in note_schedule:
        if not note.played and current_video_time >= note.start_time:
            note.played = True
            note_queue.put(note.key)
            print(f"Scheduled Note Played: {note.key} at {current_video_time:.2f}s")
